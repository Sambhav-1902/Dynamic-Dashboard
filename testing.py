"""
Test script for Qwen3-0.6B conversational resolution check.

Run this on your office laptop BEFORE using the full COM script, to verify:
1. The model downloads correctly (~500MB)
2. It loads without errors
3. It answers YES/NO correctly on realistic support ticket conversations
4. The speed is acceptable

Run:
    python test_resolution_model.py
"""
...
# ---------------------------------------------------------------------------
# Test cases — realistic support ticket conversations
# Each case has: a conversation log, the original sender, and the expected answer
# ---------------------------------------------------------------------------
TEST_CASES = [
    {
...
def run_test(model, tokenizer, conversation, sender):
    messages = [
        {
            "role": "system",
            "content": "You are a support ticket classifier. Answer YES or NO only. /no_think"
        },
        {
            "role": "user",
            "content": f"""Below is a support ticket conversation, shown NEWEST message first.
...
def main():
    print("=" * 60)
    print("Qwen3-0.6B Resolution Check — Test Suite")
    print("=" * 60)
    print()
    ...
    for i, tc in enumerate(TEST_CASES, 1):
        response, elapsed = run_test(
            model, tokenizer,
            tc["conversation"], tc["sender"]
        )
        total_time += elapsed
    ...
    if passed == len(TEST_CASES):
        ...
        print("Safe to use in track_outlook_mails_com.py.")
    elif passed >= len(TEST_CASES) * 0.75:
        print(f"Most tests passed ({passed}/{len(TEST_CASES)}).")
        print("Review the failed cases above to decide if accuracy is acceptable.")
    else:
        print(f"Too many failures ({failed}/{len(TEST_CASES)}).")
        print("Model may not be suitable — review failures above.")
